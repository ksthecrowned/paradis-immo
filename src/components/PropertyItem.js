...
const PropertyItem = (property) => {
    ...
    return (
        <View className="overflow-hidden rounded-2xl relative h-60 shadow-lg mt-3">
            <Image source={property.property.cover} className="w-full rounded-xl h-72 object-cover" ></Image>
            <LinearGradient className="absolute top-0 h-full w-full p-6 justify-end pr-12" colors={['transparent', 'rgba(0,0,0,0.1)', 'rgba(0,0,0,0.3)', 'rgba(0,0,0,0.7)']}>
                ...
                >
                    <Text className="text-xl font-bold text-white">{property.property.title}</Text>
                </TouchableOpacity>
                <Text className="text-gray-300 text-lg font-bold">{property.property.price}</Text>
                <View className="flex-row space-x-1 items-center"> 
                    <Image source={location} className="w-5 h-5"></Image>
                    <Text className="text-white">à {property.property.distance} de votre position</Text>
                </View>
            </LinearGradient>
            <View className="absolute top-6 left-6 bg-blue-500 rounded-lg py-2 px-4">
                <Text className="text-white font-bold">{property.property.objectLabel}</Text>
            </View>
            <View className="absolute right-0 w-10 h-full bg-blue-500 rounded-tl-xl mt-10 items-center py-3 space-y-4">
                <View className="items-center border-b border-white pb-1">
                    ...
                    <Text className="text-white font-bold">{property.property.nbRoom}</Text>
                </View>
                <View className="items-center border-b border-white pb-1">
                    <Image source={bathtub} className="w-6 h-6"></Image>
                    <Text className="text-white font-bold">{property.property.nbBed}</Text>
                </View>
                <View className="items-center border-b border-transparent pb-1">
                    <Image source={bed} className="w-6 h-6"></Image>
                    <Text className="text-white font-bold">{property.property.nbBath}</Text>
                </View>
            </View>
        </View>
    )
