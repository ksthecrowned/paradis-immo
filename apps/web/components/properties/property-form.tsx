...
import { lookupUserByPhone } from '@/lib/owner/leases';
import {
  listArrondissements,
  listCities,
  listQuartiers,
...
const validate = (
  ...
  opts: {
    variant: 'owner' | 'agent';
    phoneCountry: PhoneCountrySelection;
    /** Agent-on-behalf fields only required when creating. */
    requireOwnerContact?: boolean;
  },
  ...
  if (opts.variant === 'agent' && opts.requireOwnerContact !== false) {
    if (!isPhoneComplete(v.ownerPhoneNational, opts.phoneCountry)) {
      e.ownerPhoneNational = 'Numéro de téléphone du propriétaire invalide.';
    }
    e.ownerName = validateRequired(v.ownerName, 'Le nom du propriétaire') ?? '';
  }
...
const toCreateInput = (
  ...
  return {
    ...
    ...(onBehalf
      ? {
          ownerPhone: onBehalf.ownerPhone,
          ownerName: onBehalf.ownerName,
        }
      : {}),
  };
};
...
export type PropertyFormProps = {
  ...
  /** Agent creates a listing for a third-party owner (+ auto-mandate). */
  variant?: 'owner' | 'agent';
};
...
export function PropertyForm({
  ...
  const [ownerLookupHint, setOwnerLookupHint] = useState<string | null>(null);
  ...
  const form = useResourceForm<FormValues>({
    ...
    validate: (v) =>
      validate(v, {
        variant,
        phoneCountry,
        requireOwnerContact: isAgentVariant && !propertyId,
      }),
    onSubmit: async (values) => {
      if (propertyId) {
        ...
        router.push(
          isAgentVariant
            ? ROUTES.agent.property(propertyId)
            : ROUTES.owner.property(propertyId),
        );
      } else {
        const onBehalf = isAgentVariant
          ? (() => {
              const ownerPhone = getPhoneE164(
                values.ownerPhoneNational,
                phoneCountry,
              );
              if (!ownerPhone) {
                throw new Error('Numéro de téléphone du propriétaire invalide.');
              }
              return {
                ownerPhone,
          ...
          : undefined;
        const created = await createProperty(toCreateInput(values, onBehalf));
        await uploadAllPending(created.id);
        router.push(
          isAgentVariant
            ? ROUTES.agent.property(created.id)
            : ROUTES.owner.property(created.id),
        );
      }
    },
  });
  ...
  useEffect(() => {
    ...
    if (!isPhoneComplete(form.values.ownerPhoneNational, phoneCountry)) {
      setOwnerPreview(null);
      setOwnerLookupHint(null);
      return;
    }
    const e164 = getPhoneE164(form.values.ownerPhoneNational, phoneCountry);
    if (!e164) return;
    let cancelled = false;
    void (async () => {
      try {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [form.values.ownerPhoneNational, phoneCountry, isAgentVariant]);
  ...
  const tabs: FormTab[] = [
    {
      ...
      content: (
        <div className="space-y-4">
          {isAgentVariant && !propertyId ? (
            <div className="space-y-4 rounded-lg border border-border bg-muted/30 p-4">
              <p className="text-sm font-medium text-heading">
                Propriétaire du bien
              </p>
              <p className="text-sm text-muted">
                Le bien sera enregistré au nom de ce propriétaire. Votre agence
                reçoit automatiquement un mandat ; vous êtes assigné si vous êtes
                agent de terrain, sinon le gérant laisse l’affectation ouverte.
              </p>
              <FormField
                name="ownerPhoneNational"
                label="Téléphone du propriétaire"
                required
                error={form.errors.ownerPhoneNational}
              >
                <PhoneInput
                  name="ownerPhoneNational"
                  label=""
                  value={form.values.ownerPhoneNational}
                  country={phoneCountry}
                  onCountryChange={setPhoneCountry}
                  onChange={(v) => {
                    form.setField('ownerPhoneNational', v);
                    setOwnerPreview(null);
                    setOwnerLookupHint(null);
                  }}
                  invalid={!!form.errors.ownerPhoneNational}
                />
              </FormField>
              {ownerPreview ? (
                <p className="text-sm text-muted">
                  Compte trouvé : {ownerPreview.name ?? 'Sans nom'} (
                  {ownerPreview.phone})
                </p>
              ) : null}
              {ownerLookupHint ? (
                <p className="text-sm text-muted">{ownerLookupHint}</p>
              ) : null}
              <FormField
                name="ownerName"
              ...
              >
                <Input
                  ...
                  onChange={(e) => form.setField('ownerName', e.target.value)}
                  placeholder="Nom complet"
                  invalid={!!form.errors.ownerName}
                />
              </FormField>
            </div>
          ) : null}
      ...
      <DashboardPageHeader
        title={
          propertyId
            ? 'Modifier le bien'
            : isAgentVariant
              ? 'Ajouter un bien pour un propriétaire'
              : 'Ajouter un bien'
        }
      />
