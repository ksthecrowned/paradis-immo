...
export function PaymentDetail({
  ...
  const listHref =
    role === 'agent'
      ? ROUTES.agent.paymentsValidation
      : ROUTES.owner.payments;
  const dashboardHref =
    role === 'agent' ? ROUTES.agent.dashboard : ROUTES.owner.dashboard;
  const listLabel =
    role === 'agent' ? 'Validation des paiements' : 'Paiements';
  ...
  if (!payment) {
    return (
      <section className="space-y-4">
        ...
        <Link href={listHref} className="text-sm text-accent">
  ...
  return (
    <section className="space-y-6">
      <DashboardPageHeader
        ...
        breadcrumb={[
          { label: 'Paradis Immo', href: dashboardHref },
